...
class Wattpilot(object):
    ...
    def serial(self, value):
        ...
        if (self._password is not None) & (self._serial is not None):
            self._hashedpassword = base64.b64encode(
                hashlib.pbkdf2_hmac('sha512', self._password.encode(), self._serial.encode(), 100000, 256))[:32]
    ...
    def __str__(self):
        ...
        if self.connected:
            ...
            ret = ret + "Charge: " + "%.2f" % self.power + "kW" + " ---- " + str(self.voltage1) + "V/" + str(
                self.voltage2) + "V/" + str(self.voltage3) + "V" + " -- "
    ...
    def __update_property(self, name, value):
        ...
        if name == "version":
            self._version = value
        if name == "ast":
            self._AllowCharging = self.astValues[value]
        if name == "fwv":
    ...
    def __on_hello(self, message):
        logging.info("Connected to WattPilot Serial %s", message.serial)
    ...
    def __on_auth(self, wsapp, message):
        ...
        hasht = hashlib.sha256((self._token3 + message.token2 + hash1).encode()).hexdigest()
        response = dict()
        response["type"] = "auth"
    ...
    def __send(self, message, secure=False):
        # If the connection to wattpilot is over an unsecure channel (http) all send messages are wrapped in
        ...
        logging.debug("Message send: %s", json.dumps(message))
    ...
    def __on_AuthError(self, message):
        if message.message == "Wrong password":
            self._wsapp.close()
            logging.error("Authentication failed: %s", message.message)
    ...
    def __on_response(self, message):
        if message.success:
            ...
            logging.error("Error Sending Request %s. Message: %s", message.requestId, message.message)
    ...
    def __on_message(self, wsapp, message):
        ...
        if msg.type == 'response':  # Response Message -> Received after sending an update and contains result of update
